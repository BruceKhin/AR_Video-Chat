...
import com.adnet.archat.QuickSample.fragments.AudioConversationFragment;
import com.adnet.archat.QuickSample.fragments.BaseConversationFragment;
import com.adnet.archat.QuickSample.fragments.ConversationFragmentCallbackListener;
import com.adnet.archat.QuickSample.fragments.IncomeCallFragment;
import com.adnet.archat.QuickSample.fragments.IncomeCallFragmentCallbackListener;
import com.adnet.archat.QuickSample.fragments.OnCallEventsController;
import com.adnet.archat.QuickSample.fragments.VideoConversationFragment;
import com.adnet.archat.QuickSample.util.NetworkConnectionChecker;
import com.adnet.archat.Consts;
import com.adnet.archat.QuickSample.utils.FragmentExecuotr;
...
import com.adnet.archat.QuickSample.utils.RingtonePlayer;
import com.adnet.archat.QuickSample.utils.SettingsUtil;
...
public class CallActivity extends BaseActivity implements QBRTCClientSessionCallbacks, QBRTCSessionConnectionCallbacks, QBRTCSignalingCallback,
    ...
    Firebase refreshingFeatureRef = null;
    ChildEventListener featureListener = null;
    ...
    public static void start(Context context,
        ...
        ARChatApp.isReceived = isIncomingCall;
        Log.e("Receive", String.valueOf(ARChatApp.isReceived));
...
//            we have already currentSession == null, so it's no reason to do further initialization
    ...
    public void hangUpCurrentSession() {
        ...
        if(refreshingFeatureRef != null && featureListener != null){
            refreshingFeatureRef.removeEventListener(featureListener);
        }
    }
    ...
    private void addConversationFragment(final boolean isIncomingCall) {
        ArrayList<QBUser> usersFromDb = dbManager.getUsersByIds(currentSession.getOpponents());
        final ArrayList<QBUser> opponents = UsersUtils.getListAllUsersFromIds(usersFromDb, currentSession.getOpponents());
        ...
        if(ARChatApp.isReceived) {
            ARChatApp.databaseID = opponents.get(0).getLogin() + "-" +
                    SharedPrefsHelper.getInstance().getQbUser().getLogin();
            ARChatApp.mDatabase.child("CameraOption").child(ARChatApp.databaseID)
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    String str = (String)dataSnapshot.getValue();
                    ARChatApp.ant_camera_Name = str;
            ...
            ARChatApp.databaseID = SharedPrefsHelper.getInstance().getQbUser().getLogin() + "-" +
                    opponents.get(0).getLogin();
            ARChatApp.mDatabase.child(ARChatApp.databaseID).removeValue();
            ARChatApp.mDatabase.child("CameraOption").child(ARChatApp.databaseID).setValue(ARChatApp.ant_camera_Name);
            BaseConversationFragment conversationFragment = BaseConversationFragment.newInstance(
                    isVideoCall
                            ? new VideoConversationFragment()
                            : new AudioConversationFragment(),
    ...
    public void refreshFeaturesFromServer(){
        refreshingFeatureRef = ARChatApp.mDatabase.child(ARChatApp.databaseID);
        featureListener = new ChildEventListener() {
            @Override
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                FeatureItem feature = dataSnapshot.getValue(FeatureItem.class);
                byte[] matData = Base64.decode(feature.img_gray, 0);
                Mat grayMat;
                grayMat = new Mat(feature.img_height, feature.img_width, CvType.CV_8UC1);

                grayMat.put(0, 0, matData);
                if(grayMat.width() != 0) {
                    NativeFunc.OpenCMT(grayMat.getNativeObjAddr(), feature.x, feature.y, feature.width, feature.height, feature.ptArray, feature.color);
                }
            }
            ...
            public void onChildRemoved(DataSnapshot dataSnapshot) {
                FeatureItem item = dataSnapshot.getValue(FeatureItem.class);
                Log.e("ERRR", "Removed" + dataSnapshot.getKey());
                if(item != null){
                    NativeFunc.UndoCMT();
                }
            }
        ...
        refreshingFeatureRef.addChildEventListener(featureListener);
    }
}