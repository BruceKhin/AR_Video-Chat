...
public class ValidationUtils {
    ...
    private static boolean isEnteredTextValid(Context context, EditText editText, int resFieldName, int minLength) {
        ...
        if (!isCorrect) {
            editText.setError(String.format(context.getString(R.string.error_password_characters_from_app),
                    context.getString(resFieldName),
                    minLength));
            return false;
        } else {
    ...
    public static boolean isPasswordValid(Context context, EditText editText) {
        return isEnteredTextValid(context, editText, R.string.filed_user_password, 8);
    }
}
